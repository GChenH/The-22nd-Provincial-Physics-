...
QImage cvMat2QImage(const cv::Mat& mat, bool clone, bool rb_swap)
{
    ...
    else if(mat.type() == CV_32FC1)
    {
        ...
        const uchar *PSrc_32FC1 = (const uchar*)normalize_mat.data;
        QImage image(PSrc_32FC1, normalize_mat.cols, normalize_mat.rows, normalize_mat.step, QImage::Format_Grayscale8);
        return image.copy();
    ...
    else if(mat.type() == CV_32FC3)
    {
        ...
        normalize(mat, normalize_mat, 0, 255, cv::NORM_MINMAX, -1);
        normalize_mat.convertTo(normalize_mat, CV_8U);
        const uchar *PSrc_32FC3 = (const uchar*)normalize_mat.data;
        // Create QImage with same dimensions as input Mat
        QImage image(PSrc_32FC3, normalize_mat.cols, normalize_mat.rows, normalize_mat.step, QImage::Format_RGB888);
...
cv::Mat QImage2cvMat(QImage &image, bool clone, bool rb_swap)
{
    ...
    switch(image.format())
    {
        ...
        case QImage::Format_RGB888:
            ...
            if(clone)  mat = mat.clone();
            if(rb_swap) cvtColor(mat, mat, cv::COLOR_BGR2RGB);
                break;
